...
const emptyForm: ZoneForm = { name: '', min_km: '', max_km: '', fixed_price: '', sort_order: '' };
...
export default function AdminSettings() {
  ...
  const handleSave = async () => {
    if (!form.name.trim() || !form.min_km || !form.fixed_price) {
      toast.error('Nombre, km mínimo y precio son obligatorios');
      return;
    }
    setSaving(true);
    try {
  ...
  const renderForm = () => (
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 p-4 bg-muted/50 rounded-lg border">
      ...
      <div className="space-y-1">
        ...
        <Input type="number" step="0.01" value={form.fixed_price} onChange={e => setForm(f => ({ ...f, fixed_price: e.target.value }))} />
      </div>
