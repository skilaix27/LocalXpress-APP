...
export function DriverDetailDialog({ driver, stops, open, onOpenChange, onUpdate }: DriverDetailDialogProps) {
  ...
  const driverStops = useMemo(
    () => stops.filter((s) => s.driver_id === driver?.id),
    [stops, driver]
  );

  const filteredStops = useMemo(() => {
    if (stopFilter === 'all') return driverStops;
    return driverStops.filter((s) => s.status === stopFilter);
  }, [driverStops, stopFilter]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Edita la información del repartidor y consulta sus paradas asignadas.</DialogDescription>
        </DialogHeader>
        ...
        <Tabs defaultValue="edit" className="mt-2">
          ...
          <TabsContent value="edit" className="space-y-4 mt-4">
            <div className="space-y-2">
              <Label htmlFor="edit_name" className="flex items-center gap-1.5">
            ...
            <div className="space-y-2">
              <Label htmlFor="edit_phone" className="flex items-center gap-1.5">
            ...
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {form.is_active ? 'El repartidor puede iniciar sesión' : 'El repartidor está desactivado'}
                </p>
              </div>
              <Switch
          ...
          <TabsContent value="stops" className="mt-4 space-y-4">
            <Tabs value={stopFilter} onValueChange={(v) => setStopFilter(v as any)}>
              <TabsList className="w-full">
                ...
                <TabsTrigger value="picked" className="flex-1 gap-1 text-xs">
                  <Truck className="w-3 h-3" />
                  ({counts.picked})
                </TabsTrigger>
                <TabsTrigger value="delivered" className="flex-1 gap-1 text-xs">
            ...
            <div className="space-y-2 max-h-[40vh] overflow-y-auto pr-1">
              {filteredStops.length === 0 ? (
                <div className="py-8 text-center text-muted-foreground text-sm">
                  ...
                  No hay paradas con este filtro
                </div>
              ) : (
